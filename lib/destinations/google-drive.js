...
export class GoogleDriveDestination extends BaseDestination {
   ...
   * Note: The googleapis library's getAccessToken() only makes a network call
   * if the token is expired or about to expire (within 5 minutes), so this
   * timer won't cause unnecessary refreshes for recently refreshed tokens.
   */
    ...
    this.tokenRefreshTimer = setInterval(async () => {
      try {
        ...
        if (this.isRefreshing) {
          console.log(
            '⏭️  Skipping Google Drive token refresh (refresh already in progress)',
          )
          return
        }
