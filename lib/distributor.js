...
export async function distributeFile(
  ...
  if (await isHashAlreadyReceived(fileHash)) {
    console.log(
      `🔁 Duplicate file rejected: ${originalFilename} (hash: ${fileHash.substring(0, 8)}...)`,
    )
    await fs.unlink(sourcePath)
  ...
  // Log that we received this file (with hash)
  ...
  if (hasDestinations) {
    ...
    // Delete source file after successful uploads (if configured and ALL succeeded)
    if (settings.deleteAfterUpload && successful.length > 0 && failed.length === 0) {
      try {
        await fs.unlink(sourcePath)
      } catch (error) {
    ...
    return { status: 'distributed', duplicate: false }
  } else {
