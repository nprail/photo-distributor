...
/**
 * Generate a persistent random JWT base secret
 */
function generateJwtSecret() {
  return crypto.randomBytes(64).toString('hex')
}

// In Docker, lock local photos directory to /data/photos
const defaultPhotosDir = config.isDocker
  ? '/data/photos'
...
const defaultSettings = {
  ...
  // JWT signing base secret (derived with current admin password at runtime)
  jwtSecret: null,
...
export async function loadSettings() {
  try {
    ...
    if (row) {
      ...
      // Backfill JWT base secret for existing installs.
      if (!_settings.jwtSecret) {
        _settings.jwtSecret = generateJwtSecret()
...
const settings = {
  ...
  get jwtSecret() {
    return _settings?.jwtSecret || defaultSettings.jwtSecret
  },
  ...
  get destinations() {
    if (_settings?.destinations) {
      ...
      // In Docker, always force photosDir to /data/photos
      if (config.isDocker) {
        local.photosDir = '/data/photos'
      }
