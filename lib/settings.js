...
const SETTINGS_PATH = path.join(config.configDir, 'settings.json')
...
const defaultSettings = {
  ...
  destinations: {
    ...
    googleDrive: {
      ...
      tokenPath: path.join(config.configDir, 'google-drive-token.json'),
      rootFolderId: null,
    },
    googlePhotos: {
      enabled: false,
      ...
      tokenPath: path.join(config.configDir, 'google-photos-token.json'),
    },
  },
}
...
export async function loadSettings() {
  try {
    ...
    if (!_settings.ftp?.passwordHash && _settings.ftp?.password) {
      // Auto-hash the FTP password if hash is missing
...
export async function saveSettings(settings) {
  ...
  if (settings.ftp?.password) {
    // Only hash if password has changed (compare with current hash)
    const needsHashing =
      !settings.ftp.passwordHash ||
      !(await bcrypt
        .compare(settings.ftp.password, settings.ftp.passwordHash)
        .catch(() => false))

    if (needsHashing) {
      settings.ftp.passwordHash = await bcrypt.hash(settings.ftp.password, 10)
    }
  }
