...
/**
 * Get upload stats from the database
 */
export function getUploadStats() {
  const recvCol = getCollection('received')
  const destCol = getCollection('destinations')
  const all = destCol.find()
  const successful = all.filter((d) => d.success).length
  const failed = all.filter((d) => !d.success).length
  return {
    totalReceivedFiles: recvCol.count(),
