...
import { trackReceiveStart, trackReceiveEnd } from './web-server.js'
...
export class VirtualFileSystem {
  log(message) {
    ...
    console.log(`[virtual-fs] ${message}`)
  }
  ...
  constructor(connection, { SUPPORTED_EXTENSIONS }) {
    ...
    this.log('Initialized VirtualFileSystem', {
  ...
  async write(fileName, { append = false } = {}) {
    ...
    // Track file receive start
    const receiveId = trackReceiveStart(writeStream.originalFilename)
    
    writeStream.on('finish', async () => {
      // Track file receive end
      trackReceiveEnd(receiveId)
    ...
    writeStream.on('error', () => {
      // Track file receive end on error
      trackReceiveEnd(receiveId)
    })
