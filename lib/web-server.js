...
// POST /api/upload/token issues a short-lived JWT signed with the FTP password.
// Verification re-reads the current password so a password change immediately
...
async function requireUploadToken(req, res, next) {
  ...
  const secret = settings.ftpPassword
  if (!secret) {
...
export function createWebServer() {
  ...
  app.post('/api/upload/token', uploadTokenLimiter, async (req, res) => {
    try {
      ...
      const token = jwt.sign({ username }, settings.ftpPassword, {
