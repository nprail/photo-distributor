...
import { getCombinedLogs, getDestinationUploads, getReceivedById } from './upload-log.js'
...
export function createWebServer() {
  ...
  app.post('/api/retry-upload', async (req, res) => {
    try {
      ...
      if (!receivedId || !destination) {
        return res.status(400).json({ error: 'receivedId and destination are required' })
      }
      ...
      if (!received.sourcePath) {
        return res.status(400).json({ error: 'Source file path not available for this entry' })
      }
      ...
      const result = await destinationManager.uploadTo(destination, received.sourcePath, metadata)
      broadcastStatusUpdate()
  ...
  app.post('/api/retry-all-failed', async (req, res) => {
    try {
      const logs = await getCombinedLogs(200)
      const retryResults = []
      ...
      for (const file of logs) {
        ...
        const failedDestinations = (file.destinations || []).filter((d) => !d.success)
        ...
        const succeededDestinations = new Set(
          (file.destinations || []).filter((d) => d.success).map((d) => d.destination)
        )
        ...
        const uniqueFailedDests = [...new Set(failedDestinations.map((d) => d.destination))]
          .filter((d) => !succeededDestinations.has(d))

        for (const dest of uniqueFailedDests) {
          try {
            const result = await destinationManager.uploadTo(dest, file.sourcePath, metadata)
            retryResults.push({ receivedId: file.id, destination: dest, success: true, result })
          } catch (error) {
            retryResults.push({ receivedId: file.id, destination: dest, success: false, error: error.message })
          }
        }
      }
