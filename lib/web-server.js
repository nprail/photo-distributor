...
import { distributeFile } from './distributor.js'
import { PHOTO_EXTENSIONS } from './exif.js'
...
// Multer config for photo/video uploads — disk storage, no type restriction, large size limit
...
// ── Upload session tokens (JWT) ──────────────────────────────────────────────
// POST /api/upload/token issues a short-lived JWT signed with a secret derived
// from a persisted random server secret and the current admin password.
// Verification re-reads settings, so changing the admin password immediately
// invalidates all outstanding tokens.

const JWT_EXPIRY = '1h'
const JWT_ALGORITHM = 'HS256'

function getUploadJwtSecret() {
  const jwtBaseSecret = settings.jwtSecret
  const adminPassword = settings.ftpPassword

  if (!jwtBaseSecret || !adminPassword) {
    return null
  }

  return crypto
    .createHash('sha256')
    .update(jwtBaseSecret)
    .update(':')
    .update(adminPassword)
    .digest('hex')
}

// Limit token-request attempts to reduce brute-force risk
const uploadTokenLimiter = rateLimit({
...
// Limit individual file uploads to prevent resource exhaustion
...
async function requireUploadToken(req, res, next) {
  ...
  const secret = getUploadJwtSecret()
  if (!secret) {
    return res.status(500).json({ error: 'Server credentials not initialized' })
  }
...
export function createWebServer() {
  ...
  // Configure client IP resolution for reverse-proxy deployments.
  ...
  app.get('/api/init', async (req, res) => {
    try {
      ...
      // Check Drive
      try {
        await fs.access(config.googleDriveCredentialsPath)
        authStatus.drive.hasCredentials = true
      } catch {}
      try {
        await fs.access(config.googleDriveTokenPath)
      ...
      // Check Photos
      try {
        await fs.access(config.googlePhotosCredentialsPath)
        authStatus.photos.hasCredentials = true
      } catch {}
      try {
        await fs.access(config.googlePhotosTokenPath)
  ...
  // Get server-supported upload extensions so clients can stay in sync.
  app.get('/api/upload/supported-extensions', (req, res) => {
    res.json({
  ...
  // The token is valid for 1 hour and should be passed as a Bearer token
  // on subsequent POST /api/upload requests.
  app.post('/api/upload/token', uploadTokenLimiter, async (req, res) => {
    try {
      ...
      if (!settings.ftpPasswordHash) {
        return res
          .status(500)
          .json({ error: 'Server credentials not initialized' })
      }

      const valid =
        username === settings.ftpUsername &&
        (await bcrypt.compare(password, settings.ftpPasswordHash))

      if (!valid) {
        return res.status(401).json({ error: 'Invalid credentials' })
      }

      const jwtSecret = getUploadJwtSecret()
      if (!jwtSecret) {
        return res
          .status(500)
          .json({ error: 'Server credentials not initialized' })
  ...
  // Requires a valid Bearer token from POST /api/upload/token.
  app.post(
    '/api/upload',
    uploadFileLimiter,
    requireUploadToken,
    ...
    async (req, res) => {
      ...
      // Use only the basename to prevent any path traversal in the filename
      const originalFilename = path.basename(req.file.originalname)

      try {
        const result = await distributeFile(
          req.file.path,
          originalFilename,
          settings.destinations.local.photosDir,
          SUPPORTED_EXTENSIONS,
        )

        if (result?.status === 'unsupported_extension') {
          return res.status(415).json({
            success: false,
            error: `Unsupported file extension: ${result.extension}`,
            filename: originalFilename,
          })
        }

        const duplicate = result?.duplicate === true
        res.json({ success: true, filename: originalFilename, duplicate })
      } catch (error) {
        // Clean up temp file on distribution failure
        try {
      ...
      try {
        ...
        const credentialsPath =
          service === 'drive'
            ...
            : config.googlePhotosCredentialsPath
  ...
  app.get('/api/auth/google/callback', async (req, res) => {
    ...
    try {
      ...
      const tokenPath =
        service === 'drive'
          ? config.googleDriveTokenPath
          : config.googlePhotosTokenPath
